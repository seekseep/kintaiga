...
export function useMembers(parameters?: PaginationParameters, options?: { enabled?: boolean }) {
  ...
  return useQuery({
    ...
    queryFn: () => listOrganizationMembers({ data: { organizationName, parameters } }),
    ...options,
  })
}
...
export function useMember(id: string, options?: { enabled?: boolean }) {
  ...
  return useQuery({
    ...
    queryFn: () => getOrganizationMember({ data: { organizationName, memberId: id } }),
    ...options,
  })
}
...
export function useAddMember() {
  ...
  return useMutation({
    mutationFn: (body: AddOrganizationMemberInput) =>
      addOrganizationMember({ data: { organizationName, body } }),
    onSuccess: () => {
...
export function useUpdateMemberRole() {
  ...
  return useMutation({
    mutationFn: ({ id, role }: { id: string; role: 'manager' | 'worker' }) =>
      updateOrganizationMemberRole({
        data: { organizationName, memberId: id, body: { role } },
      }),
    onSuccess: (_data, { id }) => {
...
export function useDeleteMember() {
  ...
  return useMutation({
    mutationFn: (memberId: string) =>
      deleteOrganizationMember({ data: { organizationName, memberId } }),
    onSuccess: () => {
