...
function NewOrganizationPage() {
  ...
  const { mutate, isPending } = useCreateOrganization({
    onSuccess: (organization) => {
      ...
      navigate({ to: `/${organization.name}/projects` })
    },
    onError: (error: Error) => {
  ...
  const isValidName =
    name.length >= 2 && name.length <= 63 && NAME_PATTERN.test(name) && !isReservedOrganizationName(name)
            ...
            <div className="space-y-2">
              ...
              {name && !isValidName && (
                <p className="text-sm text-destructive">
                  {name.length < 2
                    ? '2文字以上で入力してください'
                    : isReservedOrganizationName(name)
                      ? 'この名前は予約されています'
              ...
              {isValidName && !isCheckingName && nameAvailable === true && (
                <p className="text-sm text-muted-foreground">URL: /{name}/projects</p>
              )}
            </div>
            <Button
              type="submit"
              disabled={!isValidName || !isValidDisplayName || !nameAvailable || isPending}
              className="w-full"
            >
              {isPending ? '作成中...' : '組織を作成'}
            </Button>
