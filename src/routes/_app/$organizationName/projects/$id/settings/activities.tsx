...
export const Route = createFileRoute('/_app/$organizationName/projects/$id/settings/activities')({
  component: ProjectActivitiesSettingsPage,
})

function ProjectActivitiesSettingsPage() {
  ...
  if (loadingProject || loadingProjectConfig || loadingConfig)
    return (
  ...
  return (
    <OrganizationRoleGuard allowedRoles={['owner', 'manager']}>
      <div className="mx-auto max-w-lg space-y-4">
        <Card>
          <CardHeader>
            <CardTitle>{project?.name} の稼働設定</CardTitle>
          </CardHeader>
          <CardContent>
            <Formik
              enableReinitialize
            ...
            >
              {({ handleSubmit, values }) => (
                <form onSubmit={(e) => { e.preventDefault(); handleSubmit() }} className="space-y-4">
                  ...
                  {(values.aggregationUnit !== 'none' && values.aggregationUnit !== USE_GLOBAL) ||
                  (values.aggregationUnit === USE_GLOBAL && globalConfig?.aggregationUnit !== 'none') ? (
