...
export const Route = createFileRoute('/_app/$organizationName/projects/$id/members/$memberId/activities')({
  component: ProjectUserActivitiesPage,
})
...
function getCurrentPeriod(_projectCreatedAt: string, unit: PeriodUnit, period: number): { start: Date; end: Date } {
  const now = new Date()
  const base = unit === 'weekly' ? startOfWeek(now, { locale: ja }) : startOfMonth(now)
...
function ProjectUserActivitiesPage() {
  ...
  if (isLoading || config === undefined || project === undefined)
    return (
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {hasPeriod && (
            <>
              ...
              <DatePicker value={effectiveStart} onValueChange={(d) => d && setPeriodStart(startOfDay(d))} />
              <span className="text-sm text-muted-foreground">-</span>
              <DatePicker value={effectiveEnd} onValueChange={(d) => d && setPeriodEnd(endOfDay(d))} />
      ...
      {dayGroups.length === 0 ? (
        <Card>
          <CardContent className="py-6 text-center text-muted-foreground">表示する日付がありません</CardContent>
        </Card>
      ) : (
                    ...
                    <TableCell colSpan={4} className="font-medium">
                      <div className="flex items-center gap-3">
                        ...
                        {group.totalMinutes > 0 && (
                          <span className="text-xs text-muted-foreground">{formatMinutes(group.totalMinutes)}</span>
                        )}
                      </div>
                    </TableCell>
                    <TableCell className="text-right">
                      <Button variant="ghost" size="sm" className="h-7 px-2" onClick={() => openAddDialog(group.date)}>
