...
export const Route = createFileRoute('/_app/$organizationName/configuration/name')({
  component: EditOrganizationNamePage,
})

function EditOrganizationNamePage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    mutation.mutate({ name }, {
      onSuccess: (data) => {
        ...
        navigate({ to: `/${data.name}/configuration` })
      },
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                />
                {name && !isValidFormat && (
                  <p className="text-sm text-destructive">英小文字、数字、ハイフンのみ（2〜63文字）</p>
                )}
                {isValidFormat && !isUnchanged && (
                  <p className="text-sm text-muted-foreground">URL: /{name}</p>
                )}
                {isChecking && (
              ...
              <Button
                type="submit"
                className="w-full"
                disabled={!isValidFormat || isUnchanged || !checkResult?.available || mutation.isPending}
              >
