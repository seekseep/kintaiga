...
export async function archiveAndDeleteUser(
  ...
  await db.transaction(async (tx) => {
    ...
    const userActivities = await tx.select().from(activities).where(eq(activities.userId, parameters.targetId))
    const userAssignments = await tx.select().from(assignments).where(eq(assignments.userId, parameters.targetId))
    ...
    if (userActivities.length > 0) {
      await tx.insert(deletedActivities).values(
    ...
    if (userAssignments.length > 0) {
      await tx.insert(deletedAssignments).values(
