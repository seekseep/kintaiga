...
describe('getAssignment', () => {
  it('アサインメントが見つかる', async () => {
    ...
    const result = await getAssignment({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { id: 'assign-1' })
    expect(result).toMatchObject({ id: 'assign-1' })
  })

  it('一般ユーザーでもアサインメントを取得できる', async () => {
    const db = createMockDb({ selectResult: [existingAssignment] })
    const result = await getAssignment({ db: db as unknown as DbOrTx }, createMemberExecutor(), { id: 'assign-1' })
    expect(result).toMatchObject({ id: 'assign-1' })
  })
  ...
  it('存在しないアサインメントは NotFoundError', async () => {
    ...
    await expect(
      getAssignment({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { id: 'nonexistent' })
  ...
  it('不正なパラメータは ValidationError', async () => {
    ...
    await expect(
      getAssignment({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { id: 123 } as unknown as { id: string })
