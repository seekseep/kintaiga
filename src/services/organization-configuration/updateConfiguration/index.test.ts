...
describe('updateConfiguration', () => {
  ...
  it('設定が存在しない場合は NotFoundError', async () => {
    ...
    await expect(
      updateConfiguration({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { roundingInterval: 30 })
  ...
  it('不正な roundingInterval は ValidationError', async () => {
    const db = createMockDb()
    await expect(
      updateConfiguration({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { roundingInterval: 7 })
  ...
  it('不正な roundingDirection は ValidationError', async () => {
    const db = createMockDb()
    await expect(
      updateConfiguration({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { roundingDirection: 'invalid' } as unknown as { roundingDirection: 'ceil' | 'floor' })
    ).rejects.toThrow(ValidationError)
  })
  ...
  it('不正な aggregationUnit は ValidationError', async () => {
    const db = createMockDb()
    await expect(
      updateConfiguration({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { aggregationUnit: 123 } as unknown as { aggregationUnit: 'monthly' | 'none' })
    ).rejects.toThrow(ValidationError)
  })
})
