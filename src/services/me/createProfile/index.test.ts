...
describe('createProfile', () => {
  it('未登録ユーザーがプロフィールを作成できる', async () => {
    ...
    const result = await createProfile({ db: db as unknown as DbOrTx, supabase: mockSupabase }, createUserExecutor(), { sub: 'sub-id', name: 'New User' })
  ...
  it('既に登録済みの場合は ConflictError', async () => {
    ...
    await expect(
      createProfile({ db: db as unknown as DbOrTx, supabase: mockSupabase }, createUserExecutor(), { sub: 'sub-id', name: 'Duplicate' })
  ...
  it('不正なパラメータは ValidationError', async () => {
    ...
    await expect(
      createProfile({ db: db as unknown as DbOrTx, supabase: mockSupabase }, createUserExecutor(), { sub: 123 } as unknown as { sub: string; name: string })
