...
describe('deleteActivity', () => {
  it('管理者は他人のアクティビティを削除できる', async () => {
    ...
    await expect(
      deleteActivity({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { id: 'act-1' })
  ...
  it('一般ユーザーは自分のアクティビティを削除できる', async () => {
    ...
    await expect(
      deleteActivity({ db: db as unknown as DbOrTx }, createMemberExecutor(), { id: 'act-1' })
    ).resolves.not.toThrow()
  })
  ...
  it('存在しないアクティビティは NotFoundError', async () => {
    ...
    await expect(
      deleteActivity({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { id: 'nonexistent' })
  ...
  it('不正なパラメータは ValidationError', async () => {
    ...
    await expect(
      deleteActivity({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { id: 123 } as unknown as { id: string })
