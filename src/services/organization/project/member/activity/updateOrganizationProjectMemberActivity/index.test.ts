...
import { updateOrganizationProjectMemberActivity } from '.'
...
describe('updateOrganizationProjectMemberActivity', () => {
  it('管理者は他人のアクティビティを更新できる', async () => {
    ...
    const result = await updateOrganizationProjectMemberActivity(
  ...
  it('一般ユーザーは他人のアクティビティを更新できない', async () => {
    ...
    await expect(
      updateOrganizationProjectMemberActivity({ db: db as unknown as DbOrTx }, createMemberExecutor(), { id: 'act-1', note: 'updated' })
  ...
  it('存在しないアクティビティは NotFoundError', async () => {
    ...
    await expect(
      updateOrganizationProjectMemberActivity({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { id: 'nonexistent', note: 'updated' })
  ...
  it('不正なパラメータは ValidationError', async () => {
    const db = createMockDb()
    await expect(
      updateOrganizationProjectMemberActivity({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { id: 123 } as unknown as { id: string })
