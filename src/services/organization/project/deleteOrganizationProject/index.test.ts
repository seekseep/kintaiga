...
import { deleteOrganizationProject } from '.'
...
describe('deleteOrganizationProject', () => {
  it('プロジェクトを削除できる', async () => {
    const db = createMockDb({ deleteResult: [deletedProject] })
    const result = await deleteOrganizationProject(
  ...
  it('存在しないプロジェクトの削除は NotFoundError', async () => {
    ...
    await expect(
      deleteOrganizationProject({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { id: 'nonexistent' })
  ...
  it('不正なパラメータは ValidationError', async () => {
    const db = createMockDb()
    await expect(
      deleteOrganizationProject({ db: db as unknown as DbOrTx }, createOwnerExecutor(), { id: 123 } as unknown as { id: string })
